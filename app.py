...
def render_facility_map():
    ...
    zone_layout = {
        "Z001": {"name":"Generator Area","x":0.5,"y":5,"w":4,"h":3.5,"type":"Production"},
        "Z002": {"name":"Fuel Unloading","x":5.5,"y":5,"w":4,"h":3.5,"type":"Logistics"},
        "Z003": {"name":"Control Room","x":10.5,"y":5,"w":3,"h":3.5,"type":"Office"},
        "Z004": {"name":"Workshop","x":0.5,"y":0.5,"w":4,"h":3.5,"type":"Maintenance"},
        "Z005": {"name":"Storage Area","x":5.5,"y":0.5,"w":4,"h":3.5,"type":"Warehouse"},
        "Z006": {"name":"Laboratory","x":10.5,"y":0.5,"w":3,"h":3.5,"type":"Testing"},
    }
    fig = go.Figure()
...
# Live Riyadh weather from Open-Meteo (free, no API key)
@st.cache_data(ttl=600)  # Cache 10 minutes
def get_riyadh_weather():
    try:
        url = "https://api.open-meteo.com/v1/forecast?latitude=24.7136&longitude=46.6753&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&timezone=Asia/Riyadh"
        ...
        if wc == 0: icon, desc = "☀️", "Clear"
        elif wc in [1,2,3]: icon, desc = "⛅", "Partly Cloudy"
        elif wc in [45,48]: icon, desc = "🌫️", "Foggy"
        elif wc in [51,53,55,61,63,65]: icon, desc = "🌧️", "Rain"
        elif wc in [71,73,75,77]: icon, desc = "❄️", "Snow"
        elif wc in [80,81,82]: icon, desc = "🌦️", "Showers"
        elif wc in [95,96,99]: icon, desc = "⛈️", "Thunderstorm"
        else: icon, desc = "🌤️", "Fair"
        return icon, f"{desc} · {temp}°C", f"💧 {humidity}% · 💨 {wind} km/h"
    except:
        # Fallback to season estimate
        m = datetime.now().month
        if m in [6,7,8]: return "☀️", "Hot · ~45°C", ""
        elif m in [12,1,2]: return "🌤️", "Mild · ~20°C", ""
        elif m in [3,4,5]: return "⛅", "Warm · ~32°C", ""
        else: return "🌤️", "Warm · ~35°C", ""

wx_icon, wx_txt, wx_detail = get_riyadh_weather()
...
<div class="nav-bar">
    ...
    <div style="display:flex;flex-direction:column;align-items:center;padding:8px 16px">
        ...
        <div style="color:rgba(255,255,255,0.4);font-size:10px">{wx_detail}</div>
...
with tab1:
    ...
    clim_cols = st.columns(len(zones_df))
    for i, (_, z) in enumerate(zones_df.iterrows()):
        zid = z["ZoneID"]
        zn = z["ZoneName"]
        # Get temp, humidity, heat index for this zone
        t_df = readings_df[(readings_df["ZoneID"]==zid) & (readings_df["HazardType"]=="Temperature")]
        h_df = readings_df[(readings_df["ZoneID"]==zid) & (readings_df["HazardType"]=="Humidity")]
        hi_df = readings_df[(readings_df["ZoneID"]==zid) & (readings_df["HazardType"]=="HeatIndex")]
        t_val = round(float(t_df["MeasuredValue"].mean()), 1) if len(t_df) > 0 and not pd.isna(t_df["MeasuredValue"].mean()) else None
        h_val = round(float(h_df["MeasuredValue"].mean()), 1) if len(h_df) > 0 and not pd.isna(h_df["MeasuredValue"].mean()) else None
        hi_val = round(float(hi_df["MeasuredValue"].mean()), 1) if len(hi_df) > 0 and not pd.isna(hi_df["MeasuredValue"].mean()) else None
        # Estimate if missing
        if t_val is None and hi_val is not None: t_val = round(hi_val * 0.85, 1)
        if h_val is None and hi_val is not None: h_val = round(40 + (hi_val - 30) * 2, 1)
        if hi_val is None and t_val is not None and h_val is not None:
            hi_val = round(t_val + (h_val * 0.1), 1)
        t_str = f"{t_val}°C" if t_val else "N/A"
        h_str = f"{h_val}%" if h_val else "N/A"
        hi_str = f"{hi_val}" if hi_val else "N/A"
        hi_color = "#C62828" if hi_val and hi_val >= 40 else "#F9A825" if hi_val and hi_val >= 32 else "#2E7D32"
        with clim_cols[i]:
            st.markdown(f'''<div style="background:#0F172A;border:1px solid #334155;border-radius:12px;padding:10px;text-align:center;min-height:100px">
                <div style="color:{C["text1"]};font-size:11px;font-weight:700;margin-bottom:6px">{zn}</div>
                <div style="color:{C["text2"]};font-size:11px">🌡️ {t_str}</div>
                <div style="color:{C["text2"]};font-size:11px">💧 {h_str}</div>
                <div style="color:{hi_color};font-size:13px;font-weight:800;margin-top:4px">🔥 {hi_str}</div>
            </div>''', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
def generate_local_answer(question, data_context, is_arabic=False):
    ...
    if any(w in q for w in ["equation", "معادل", "formula", "صيغ", "حساب", "calculation"]):
        if is_arabic:
            return """## 📐 معادلات ExpoInsight

### 1️⃣ المعادلة الأساسية — نسبة التعرض
```
نسبة التعرض (%) = القراءة الحالية ÷ الحد المسموح × 100
```
**مثال:** CO₂ = 621 ppm ÷ 1000 ppm = **62%** (آمن)
...
### 3️⃣ الضوضاء — الجمع اللوغاريتمي
```
L = 10 × log₁₀(10^(L₁/10) + 10^(L₂/10))
```
الضوضاء **لا تُجمع عادي**. مثال: 85 dBA + 80 dBA = **86.2 dBA** (مو 165)

### 4️⃣ الحرارة — نموذج التأثير الحراري
```
ΔT = (حرارة المعدة - حرارة المنطقة) × 0.08
الحرارة الجديدة = حرارة المنطقة + ΔT
```
**المعامل 0.08** قابل للمعايرة — يمثل نسبة التأثير الفعلي للمعدة على المنطقة. معدة 80° في زون 40° ترفعه لـ **43.2°** (مو 80°).

### 5️⃣ الغازات وCO₂ — التراكم المباشر
```
التركيز الجديد = التركيز الحالي + انبعاث المعدة
```
الغازات تتراكم في الهواء مباشرة بالـ ppm.

### 6️⃣ مؤشر الحرارة — معادلة NOAA/NWS
يُحسب من **الحرارة + الرطوبة** باستخدام معادلة Rothfusz الرسمية المعتمدة من OSHA.

### 7️⃣ المعايير المرجعية
- **OSHA PEL** — CO₂: 1,000 ppm، الضوضاء: 85 dBA، الغازات: 25 ppm
- **ACGIH TLV** — مؤشر الحرارة: 40"""
        else:
            return """## 📐 ExpoInsight Equations

### 1️⃣ Core Formula — Exposure Percentage
```
Exposure % = Current Reading ÷ Permissible Limit × 100
```
**Example:** CO₂ = 621 ppm ÷ 1000 ppm = **62%** (Safe)
...
### 3️⃣ Noise — Logarithmic Addition
```
L = 10 × log₁₀(10^(L₁/10) + 10^(L₂/10))
```
Noise levels **cannot be added linearly**. Example: 85 dBA + 80 dBA = **86.2 dBA** (not 165)

### 4️⃣ Heat — Thermal Impact Model
```
ΔT = (Equipment Temp - Zone Temp) × 0.08
New Zone Temp = Zone Temp + ΔT
```
The **0.08 factor** is a tunable thermal impact coefficient representing the actual heat radiation effect on the zone. Equipment at 80°C in a 40°C zone raises it to **43.2°C** (not 80°C).

### 5️⃣ Gas & CO₂ — Additive Concentration
```
New Concentration = Current + Equipment Emission
```
Gas concentrations accumulate directly in ppm.

### 6️⃣ Heat Index — NOAA/NWS Formula
Calculated from **Temperature + Humidity** using the official Rothfusz regression equation adopted by OSHA.

### 7️⃣ Regulatory Standards
- **OSHA PEL** — CO₂: 1,000 ppm, Noise: 85 dBA, Gas: 25 ppm
